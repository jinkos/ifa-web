...
export default function BalanceBuyToLetEditor({
  ...
  const displayRent = (rental.periodic_amount ?? 0) === 0 && rental.periodic_amount !== null ? '' : (rental.periodic_amount ?? '');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Field label="Property value">
          <Input
            type="number"
            inputMode="numeric"
            value={(item.ite.property_value ?? '') as any}
            placeholder="value"
            onChange={(e) => {
              const v = e.target.value === '' ? null : Math.round(Number(e.target.value));
              onChange({ ...item, ite: { ...item.ite, property_value: v } });
            }}
          />
        </Field>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Field label="Rent">
          <Input
            type="number"
            inputMode="numeric"
            value={displayRent as any}
            placeholder="amount"
            onChange={(e) => {
              const v = e.target.value === '' ? null : Math.round(Number(e.target.value));
              onChange({ ...item, ite: { ...item.ite, rental_income: { ...rental, periodic_amount: v } } });
            }}
          />
        </Field>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Field label="Repayment">
          <Input
            type="number"
            inputMode="numeric"
            value={((item.ite.loan?.repayment?.periodic_amount ?? null) === null || (item.ite.loan?.repayment?.periodic_amount ?? 0) === 0) ? '' : (item.ite.loan?.repayment?.periodic_amount as any)}
            placeholder="amount"
            onChange={(e) => {
              const v = e.target.value === '' ? null : Math.round(Number(e.target.value));
              const loan = ensureLoan();
              const repay = ensureRepayment();
              onChange({ ...item, ite: { ...item.ite, loan: { ...loan, repayment: { ...repay, periodic_amount: v } } } });
            }}
          />
        </Field>
        <Field label="Frequency">
