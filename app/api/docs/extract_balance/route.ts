...
// POST /api/docs/extract_balance
// Body: { team_id: number|string, client_id: number|string }
// Proxies to FastAPI /docs/extract_balance and returns its JSON response
export async function POST(req: Request) {
  const body = await req.json();
  try {
    const r = await fetch(`${FASTAPI_URL}/docs/extract_balance`, {
      method: 'POST',
